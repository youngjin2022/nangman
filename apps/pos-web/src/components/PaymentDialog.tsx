...
export function PaymentDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/50 animate-fade-in">
      <div className="w-full max-w-md bg-bg-panel rounded-2xl shadow-xl animate-slide-up">
        <div className="p-5 border-b border-line">
          <h2 className="text-lg font-bold">결제 처리</h2>
          <p className="text-sm text-ink-muted mt-1">결제 수단을 선택하세요</p>
        </div>
        <div className="p-5">
          <div className="p-4 bg-bg-subtle rounded-xl mb-5">
            <div className="text-xs text-ink-muted">총 결제금액</div>
            <div className="text-2xl font-bold text-accent tabular-nums mt-1">
              {formatKRW(amount)}
            </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {METHODS.map((m) => (
              <button
                ...
                className={cn(
                  'h-14 rounded-xl border-2 flex items-center justify-center gap-2 text-sm font-medium',
                  method === m.id
        ...
        <div className="p-4 border-t border-line grid grid-cols-2 gap-2">
          <button
            onClick={onClose}
            disabled={processing}
            className="h-12 rounded-xl bg-bg-subtle font-medium disabled:opacity-50"
          >
            취소
          </button>
          <button
            ...
            className="h-12 rounded-xl bg-accent text-white font-semibold disabled:opacity-50 hover:bg-accent-dark"
          >
            {processing ? '결제 처리 중…' : `${formatKRW(amount)} 결제`}
          </button>
        </div>
      </div>
    </div>
